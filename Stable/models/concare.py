...
class ConCare(nn.Module):
    ...
    def forward(self, input_data, choice):
        ...
        time_context = torch.zeros(input_data.shape[0], input_data.shape[1], self.hidden_dim).to(device)
        ...
        for j in range(input_data.size(1)):
            ...
            time_context[:, j] = weighted_contexts
            time_output[:, j] = output

        return time_output, time_context
